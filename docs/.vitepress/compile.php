...
if ($is_inline) {
	...
	ob_start();
	$uses_for_inline = array_map(function($use) {
			return "use {$use};";
	}, $uses);
	try {
...
if ($is_blade) {
	$blade = new BladeOne();
	$uses_for_blade = array_map(function($use) {
			return "@use({$use})";
	}, $uses);
...
$uses_for_block = array_map(function($use) {
		return "use {$use};";
}, $uses);
...
foreach ($statements as &$stmt) {
	...
	try {
		...
		if (!in_array($stmt['type'], array('Stmt_Echo', 'Expr_StaticCall', 'Expr_FuncCall', 'Stmt_TryCatch', 'Stmt_Foreach', 'Stmt_While'))) {
			continue;
		}
